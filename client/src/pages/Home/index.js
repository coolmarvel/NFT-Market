    ...
    const init = async () => {
      try {
        ...
        try {
          ...
          alert(
            "최신 네트워크에 배포되지 않은 Contract " + networkId.toString()
          );
        }
      } catch (error) {
        alert(
          `Web3, Account, Contract를 로드하지 못했습니다. 자세한 내용은 콘솔을 확인하십시오.` +
            error
        );
        console.error(error);
      }
  ...
  return (
    <div className={classes.homepage}>
      <section className={classes.banner}>
        <Grid container spacing={0} xs={12} className={classes.gridBanner}>
          ...
          <Grid item xs={6} className={classes.main}>
            ...
            <Typography>
              당신의 예술작품을 전시할 수 있는 분산형 NFT 마켓플레이스입니다.
            </Typography>
            <Link to="/create-nft">
      ...
      <section className={classes.allNfts}>
        <Typography className={classes.title}>최신 작품</Typography>
        <Grid
          container
          direction="row"
